...
Design contract:
    ...
    * **Single source of truth for capacity**: physical cap values live
      ...
      :class:`vantage.domain.ISLEdge` (``capacity_gbps``),
      :class:`vantage.domain.ShellConfig` (``feeder_capacity_gbps``),
      :class:`vantage.domain.GroundStation` (``max_capacity``).
    ...
    * **Per-epoch accounting**: :class:`UsageBook` is the only mutable
      class in :mod:`vantage.domain`. It holds a reference to a
...
from vantage.domain.ground import GroundStation
from vantage.domain.satellite import SatelliteState, ShellConfig
