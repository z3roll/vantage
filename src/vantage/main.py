...
def main() -> None:
    ...
    # ── GroundKnowledge: starts EMPTY, per-PoP capacity ────
    from vantage.world.ground.knowledge import LRUEviction
    gk = GroundKnowledge(pop_capacity=100, eviction=LRUEviction())
    ...
    # ── Summary ─────────────────────────────────────────────
    print(f"\nGroundKnowledge: {gk.total_size()} entries")
    pops_with_data = sum(1 for p in ground.pops if gk.pop_size(p.code) > 0)
