...
Generators implement the :class:`TrafficGenerator` protocol —
``generate(epoch) → TrafficDemand``.

Four generators, from simplest to most realistic:

1. :class:`UniformGenerator` — all-to-all, fixed demand.
2. :class:`GravityGenerator` — demand ∝ 1/distance, static.
3. :class:`RealisticGenerator` — Zipf destinations, gradual discovery.
4. :class:`FlowLevelGenerator` — Gravity spatial, Bounded Pareto flow
   sizes, Poisson arrivals with diurnal modulation, per-flow lifecycle.
"""
...
# ---------------------------------------------------------------------------
# FlowLevelGenerator — analytical per-city demand aggregation
# ---------------------------------------------------------------------------
